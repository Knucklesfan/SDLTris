...
// property Image texture0: Image { source: "./Shader_Fractal_Flythrough_texture0.png" }
...
void main(){
    ...
    vec3 up = (cross(fwd, rgt));
        ...
        // Unit direction ray.
    vec3 rd = normalize(fwd + FOV*((u.x-0.5)*rgt + (u.y-0.5)*up));
    ...
    if(t<FAR){
        ...
        nor = texBump(texture0, pos*ts, nor, 0.002); // + step(saveObjID, 1.5)*0.002
        ...
        col = tex3D(texture0, pos*ts, nor); // Texture pixel at the scene postion.
        ...
        vec3 rCol = tex3D(texture0, rsp*ts, rsn); // Texel at rsp.
        vec3 rLi = lp-rsp;
